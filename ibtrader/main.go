				...
				if down > 0 {
					...
					if downMap[symbol] != nil {
						...
						if downtimes == 4 && downMap[symbol].trigger >= 3 {
							str += "进入不波动买点 " + fmt.Sprintf("%.2F", item[4].(float64)) + normalTime.Format("2006-01-02 15:04:05 MST") + "\n"
							downMap[symbol].trigger = -1
						} else if downMap[symbol].low > item[3].(float64) && downtimes > 4 && diff > 0.0095 && downMap[symbol].trigger >= -1 {
							str += "进入波动买点1 " + fmt.Sprintf("%f, %f", item[4].(float64), diff) + normalTime.Format("2006-01-02 15:04:05 MST") + "\n"
							...
							str += "进入波动买点2 " + fmt.Sprintf("%f, %f", item[4].(float64), diff) + normalTime.Format("2006-01-02 15:04:05 MST") + "\n"
						} else if downMap[symbol].low > item[3].(float64) && downtimes > 4 && diff > 0.0395 && downMap[symbol].trigger == -3 {
							downMap[symbol].trigger = -4
							str += "进入波动买点3 " + fmt.Sprintf("%f, %f", item[4].(float64), diff) + normalTime.Format("2006-01-02 15:04:05 MST") + "\n"
						}
					}
