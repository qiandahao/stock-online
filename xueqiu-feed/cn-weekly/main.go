...
func main() {
	...
	weekOfMonth := currentWeek - firstWeek + 1

	// 打印结果
	formattedDate = formattedDate + "#" + strconv.Itoa(weekOfMonth)
	fmt.Println("Formatted Date:", formattedDate)
	folder_path := "D:\\data_volume\\json\\cn\\weekly\\" + formattedDate
	...
	for _, symbol := range symbols {
		...
		dailyDataUrl := "https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol=" + symbol + "&begin=" + strconv.FormatInt(unixMilli, 10) + "&period=week&type=before&count=-4000&indicator=kline,pe,pb,ps,pcf,market_capital,agt,ggt,balance"
