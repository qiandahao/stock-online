...
func main() {
	...
	folder_path := "D:\\data_volume\\json\\us\\daily\\" + formattedDate
	...
	url = "https://stock.xueqiu.com/v5/stock/screener/quote/list.json?size=100&order=desc&order_by=symbol&market=US&type=us"
	for i := 0; i < 50; i++ {
		temp := url + "&page=" + strconv.Itoa(i)
		req, _ := http.NewRequest("GET", temp, nil)
		req.Header.Set("Content-Type", "application/json")
